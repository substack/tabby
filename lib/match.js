...
Match.prototype.add = function (pattern, route) {
    ...
    route._events = new EventEmitter;
    var regex = RegExp(
        ('(' + quotemeta(pattern) + ')').replace(/\\:([\w-]+)/g, '([\\w-]+)')
    );
    route._events.regex = regex;
    route._events.test = function (s) { return regex.test(s) };
